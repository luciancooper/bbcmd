...
class BBIndex2():
    ...
    def _new_inst(c,i,v,dtype,**kwargs):
        ...
        inst.dtype,inst.i,inst.v = dtype,i,v
    ...
    def _extendIndexBranch(self,j,spans):
        ...
        """
        if len(self.i[j])==0:
            self.i[j].extend([0]+[*self._offset_spans([len(x) for x in i],0)])
        else:
            self.i[j].extend([*self._offset_spans([len(x) for x in i],self.i[j][-1])])
        """
    ...
    def _subIndex(self,value):
        ...
        for (i,v) in enumerate(value):
            ...
            if i+1 == len(self.i):
                ...
                j1 = binaryIndex(self.i[i+1],self.i[i][x])+1
